...
function defaultData() {
  return {
    ...
    expenses: [],
    targets: { monthlySavings: 0, netWorthTarget: 0, netWorthTargetDate: "" },
    netWorthHistory: [],
    bonusResults: [],
  };
}
...
export default function FinanceTracker() {
  ...
  const derived = useMemo(() => {
    ...
    const savingsGoal = Number(data.targets.monthlySavings || 0);
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 16 }}>
        ...
        <MetricCard label="Savings goal" value={fmt(derived.savingsGoal)} />
...
function NetWorthTab({ data, update, derived }) {
  ...
  const target = Number(data.targets.netWorthTarget || 0);
  const progressPct = target > 0 ? (derived.latestNetWorth / target) * 100 : 0;
  const sortedHistory = [...data.netWorthHistory].sort((a, b) => a.date.localeCompare(b.date));
  ...
  return (
    <div>
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 16 }}>
        ...
        <MetricCard label="Target" value={fmt(target)} sub={data.targets.netWorthTargetDate ? `by ${data.targets.netWorthTargetDate}` : undefined} />
        <MetricCard label="Progress to target" value={target > 0 ? `${Math.round(progressPct)}%` : "—"} tone={progressPct >= 100 ? "good" : undefined} />
      </div>
        ...
        <div style={{ height: 220 }}>
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={derived.netWorthPoints} margin={{ left: 0, right: 20, top: 10 }}>
              ...
              {target > 0 && <ReferenceLine y={target} stroke={RUST} strokeDasharray="5 4" label={{ value: "Target", position: "insideTopRight", fill: RUST, fontSize: 11 }} />}
    ...
    <div>
      ...
      <Card>
        ...
        <div style={{ fontSize: 14 }}>
          ...
          {monthlyRate > 0 && target <= 0 && "Set a net worth target on the Setup tab to see a projected date."}
          {monthlyRate > 0 && target > 0 && monthsToTarget !== null && (
            monthsToTarget === 0
              ? "You've already reached your target."
              : `At ${fmt(monthlyRate)} a month, you'd reach your target of ${fmt(target)} in about ${monthsToTarget} month${monthsToTarget === 1 ? "" : "s"}.`
          )}
        </div>
      </Card>
    </div>
...
function SetupTab({ data, update }) {
  ...
  const setTarget = (field, v) => update((d) => { d.targets[field] = field === "netWorthTargetDate" ? v : (v === "" ? "" : Number(v)); });
      ...
      <Card>
        ...
        <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
          <Field label="Monthly savings goal">
            <input style={inputStyle} type="number" value={data.targets.monthlySavings} onChange={(e) => setTarget("monthlySavings", e.target.value)} />
          </Field>
          <Field label="Net worth target">
            <input style={inputStyle} type="number" value={data.targets.netWorthTarget} onChange={(e) => setTarget("netWorthTarget", e.target.value)} />
          </Field>
          <Field label="Target date (optional)">
            <input style={inputStyle} type="date" value={data.targets.netWorthTargetDate} onChange={(e) => setTarget("netWorthTargetDate", e.target.value)} />
          </Field>
        </div>
      </Card>
    </div>
